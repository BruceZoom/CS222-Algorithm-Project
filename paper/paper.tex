...
After finetune, most of the classifiers can reach an accuracy more than 80\%, which is much higher than the overall accuracy of the original full network.
...
The accuracy of the four classifiers are shown in Table~\ref{tab:classifier-accuracy}.The test set has also been balanced to make the number of positive and negative samples nearly $1:1$. 
...
For instance, if the result of classifier-3 is $0$, we don't need to take other classifiers into consideration since the only class $0$ is possible in cluster-3 according to Table~\ref{tab:cluster_label}
