...
\begin{abstract}
  To deploy DNN models in edge devices, we need to reduce the size and complexity of the model to fit the limited energy supply and computational capability of those devices.
  One solution to the problem is to substitute the large network with small ones and focus only on specific tasks.
  To give an implementation of such idea, we propose a general framework of assembling multiple binary classifiers into one that performs complicated classification tasks and give an heuristic search algorithm for the classifier combination.
  We instantiate such framework on network fusion for image classification using critical data routing path method, and we implement it to fuse two VGGNets w.r.t. part of CIFAR-100 dataset.
  Experiments show that we can reduce about $90\%$ computing consumption by pruning convolution kernels in networks while maintain a high classification accuracy for partial classification. \footnote{The github repository for the project is \url{https://github.com/BruceZoom/CS222-Algorithm-Project.git}.}
\end{abstract}
...
\Cref{fig:assemble-example-encoding} shows the encoding mentioned in the certificate algorithm for the intro example in \Cref{fig:assemble-example-venn}.
...
The sample distribution of each cluster is shown in \Cref{fig:vgg-cluster}.

\begin{figure}[h]
    \centering
    \begin{minipage}{0.99\linewidth}
		\subfigure[Clusters from VGG-C]{
			\includegraphics[width=\linewidth]{fig/chart1.pdf}
			\label{fig:net2a}
		}
    \end{minipage}
    ...
    \begin{minipage}{0.99\linewidth}
		\subfigure[Clusters from VGG-D]{
			\includegraphics[width=\linewidth]{fig/chart2.pdf}
			\label{fig:net2b}
		}
    \end{minipage}
