...
class SubclassEncoder(object):
    def __init__(self, data, labels, cluster_per_class=2, threshold=None, verbose=0):
        ...
        kmeans = KMeans(n_clusters=int(len(classes) * cluster_per_class),  verbose=verbose).fit(data)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--center_save_path",help="Path to save centers",
    ...
    if args.prog == 'encode':
        ...
        if args.encoder_save_path is not None:
            ...
            outfile.close()
        for kmeans in encoder.encoders:
            cluster_centers = kmeans.cluster_centers_
            for k,center in enumerate(cluster_centers):
                layer_center = idx2layer(center)
                outfile = os.path.join(args.center_save_path,"cluster%d.json"%k)
                with open(outfile,"w") as f:
                    json.dump(layer_center,f)
