...
Recent years have witnessed the rapid development of deep learning(DL). Due to the irreplaceable roles of these models in data processing, it is promising to implement it on these edge device such as wearable device. However, limited energy supply and computing capabilities on edge device lead to constraint on the scale and depth of the deep learning model. Thus, light-weight models are required to fit these features of edge device. 
	
In this project, we propose a combination of light-weight neural networks to substitute the original large deep learning model. With this new model, we will try to reduce the parameter number and computing consumption notably at the expense of acceptable accuracy decline compared to the original large model.

% xyc
\section{Problem Description}
We mainly deal with the issue to replace a large deep learning model with a set of small models which must be featured with less depth. Moreover, the number of small models cannot be too large. Equivalently, the sum of parameter number in all small models must be much less than the original model. Then, the total computing consumption can decrease definitely. As a result, edge device can profit from the lower demand for implementation platforms. And these small models are independent from the original deep learning network at least in the inference phase. Hence, they can process the data on the edge device without communiction with the cloud or servers, which extend the application range of these device.

We will take the example of image recognition. At present, AdaBoost tends to be one possible solution. However, the weaker learners in these algorithm, which may be very simple classifiers, are so susceptible that the outcome is senstitive to noisy data and outliers. By contrast, thanks to the application of neural networks and more sophiscated combination methods, we will attempt to pursue a more robust and accurate model.
